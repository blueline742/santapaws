...
function initImageCalendar() {
    const imageDoors = document.querySelectorAll('.calendar-grid-image .door');
    
    if (imageDoors.length === 0) return; // Exit if no image calendar found
    
    // Get current day for advent calendar logic
    const today = new Date();
    const currentDay = 25; // Set to 25 for testing - allows all doors to be opened
    // For production, use: const currentDay = today.getDate();
    
    imageDoors.forEach(door => {
        ...
        door.addEventListener('click', function() {
            ...
            if (day > currentDay) {
                showModal({
                    ...
                    message: `Day ${day} will be unlocked on December ${day}!`,
            ...
            // Show prize in modal
            const prize = door.dataset.prize;
            showModal({
                icon: '🎁',
                title: 'Congratulations!',
                message: `You opened Day ${day} and won:`,
                prize: prize,
                showClaimButton: true
            });
            
            // Save opened state to localStorage
            localStorage.setItem(`image-door-${day}`, 'opened');
        });
        
        // Restore opened state from localStorage
        if (localStorage.getItem(`image-door-${day}`) === 'opened') {
            door.classList.add('opened');
        }
        
        // Add hover effect for available doors
        if (day <= currentDay) {
...
function showModal(options) {
    ...
    if (options.prize) {
        content += `<h3 style="color: #FFD700; font-size: 1.8rem; margin: 1rem 0;">${options.prize}</h3>`;
    }
    
    if (options.showClaimButton) {
        content += `
            <a href="https://t.me/Santapawssolofficial" target="_blank" class="btn btn-primary" style="margin-top: 1rem; display: inline-block;">
                Join Telegram to Claim
            </a>
        `;
    }
...
// Initialize image calendar when DOM is loaded
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initImageCalendar);
} else {
    initImageCalendar();
}
